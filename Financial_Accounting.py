...
class Tools:
    def write_new(self):
        with open('Storage_Current.py', 'w') as storage:
            storage.write('dbr = {}\ncbr = {}\ndje = []\nret_earn = 0\nni = 0')
        first = str(ma.hash('originoriginoriginoriginoriginoriginoriginoriginoriginoriginoriginorigin'))
        ...
        with open('Storage_Journal.py', 'w') as journal:
            journal.write(f'{first} Blank Data Not Linked to Hash!\ndbr = {""}\ncbr = {""}\ndje = []\nret_earn = 0\nni = 0\n')
...
class VerifyIntegrity:
    def verifyinternal(self):
        ...
        from Storage_Current import dbr, cbr, dje, ret_earn, ni
        data = f'>{dbr}%{cbr}#{dje}@{ret_earn}!{ni}<'
        verified = ma.hash(b.components(Bank[-2], data, dje[-1]))
        if verified == Bank[-1]:
    ...
    def addblock(self):
        ...
        with open('Storage_Journal.py', 'a') as jstorage:
            jstorage.write(f'\n{str(Bank[-1])}\ndbr = {dbr}\ncbr = {cbr}\ndje = {dje}\nret_earn = {ret_earn}\nni = {ni}\n')
            jstorage.close()
        finaldata = f'{Bank[-2]}{data}{dje[-1]}*{Bank}'
        return finaldata
